...
import com.shure.surdes.survey.mapper.*;
...
public class AnswerJsonServiceImpl implements IAnswerJsonService {
    ...
    @Autowired
    private MbtiMatchMapper mbtiMatchMapper;
    ...
    public JSONObject selectAnswerJsonLatest(AnswerJson answerJson) {
        ...
        String uid = answerJson.getWbUid();
        log.info("查询用户最新答案传过来的用户id:{},surveyId:{},stype:{},微博uid:{}", userId, surveyId, stype, uid);
...
//        ii
        ...
        if (SurveyType.MBTI_AI_SURVEY_TEST.equals(stype)) { // ai测试结果
            ...
            // 查询用户的订单信息
            selectOrder(userId, String.valueOf(surveyId));
            ...
            // 订单结果
            result = answerJsonMapper.selectAnswerJsonByAnId(anId);
        } else if (SurveyType.MBTI_AI_PK.equals(stype)) { // disc pk测试结果
            if (StringUtils.isEmpty(list)) {
                json.put("code", 500);
                json.put("msg", "无MBTI大PK，请前往MBTI大PK测试进行测试！");
                return json;
            }
            // 查询用户的订单信息
            selectOrder(userId, String.valueOf(surveyId));
            ...
            // 查询订单信息
            List<SurveyOrder> orderList = surveyOrderService.list(sowrapper);
//            surveyOrder=selectOrder( userId,  surveyId);
            if (StringUtils.isEmpty(orderList)) {
                json.put("code", 500);
                json.put("msg", "没有支付订单信息，无法查看MBTI大PK测试结果！");

//                LambdaQueryWrapper<SurveyOrder> sowrapper2 = new LambdaQueryWrapper<SurveyOrder>();
//                sowrapper2.eq(SurveyOrder::getUserId, userId);
//                sowrapper2.eq(SurveyOrder::getSurveyId, surveyId);
//                sowrapper2.orderByDesc(SurveyOrder::getCreateTime);
//                sowrapper2.last("limit 1");
//                SurveyOrder surveyOrder1=surveyOrderService.getOne(sowrapper);
//                if(surveyOrder1!=null){
//                    json.put("data",surveyOrder1.getStarUid())
//                }
                return json;
            }
            // 最新的订单
        ...
        if (StringUtils.isNotEmpty(list)) {
            ...
            if (SurveyType.MBTI_AI_SURVEY_TEST.equals(surveyType)) { // ai测试结果
            ...
            } else if (SurveyType.MBTI_AI_PK.equals(surveyType)) {
                //用户部分
                log.info("查询MBTI大PK测试结果,开始封装得分数据 : " + origin);
                JSONObject aiJson = JSONObject.parseObject(origin);
                for (String chara2 : character4Type) {
                    String[] split = chara2.split("");
                    Map<String, Object> map11 = new HashMap<>();
                ...
                String starUid = surveyOrder.getStarUid();
                AnswerJson starJson = answerJsonMapper.selectAnswerJsonByWbUid(starUid);

                if (starJson == null) {

                }
                // 用户得分
                String starOrigin = starJson.getAnswerResultOrigin();
                // 用户性格结果
                String starAnswerResult = starJson.getAnswerResult();
                String starSurveyType = starJson.getSurveyType();
                json.put("starAnswer", starJson);
                LambdaQueryWrapper<MbtiMatch> mbtiWrapper = new LambdaQueryWrapper<>();
                mbtiWrapper.eq(MbtiMatch::getUserMbti, answerResult)
                        .eq(MbtiMatch::getStarMbti, starAnswerResult);
                // TODO: 2024/10/18
                MbtiMatch mbtiMatch = mbtiMatchMapper.selectOne(mbtiWrapper);

                JSONObject mbtiMatchJSON = JSON.parseObject(JSONObject.toJSONString(mbtiMatch));
                LambdaQueryWrapper<StarAnswer> answerLambdaQueryWrapper = new LambdaQueryWrapper<>();
                //获取明星信息
                answerLambdaQueryWrapper.eq(StarAnswer::getStarUid, starUid);
                StarAnswer starAnswer = starAnswerService.getOne(answerLambdaQueryWrapper);

                SysUser sysUser = sysUserService.selectUserById(surveyOrder.getUserId());

                mbtiMatchJSON.put("starImg", Constants.Star_Img_PREFIX + "/" + starAnswer.getStarImg());
                mbtiMatchJSON.put("starName", starAnswer.getStarName());
                mbtiMatchJSON.put("userImgBase64", sysUser.getBigAvatarBase64());
                mbtiMatchJSON.put("userName", sysUser.getUserName());

                LambdaQueryWrapper<MbtiTypeDesc> wrapper = new LambdaQueryWrapper<MbtiTypeDesc>();
                wrapper.eq(MbtiTypeDesc::getCharaCode, starAnswerResult);
                MbtiTypeDesc mbtiTypeDesc = mbtiTypeDescMapper.selectOne(wrapper);

                String starMbtiLoveDesc = mbtiTypeDesc.getLoveGuide();
                String loveResult = starMbtiLoveDesc.replace("你", "他");
                mbtiMatchJSON.put("starLoveGuide", loveResult);

                json.put("mbtiMatch", mbtiMatchJSON);
    ...
    private JSONObject getAiTestByTimeResult(AnswerJson answerJson) {
        try {
            ...
            String uid = answerJson.getWbUid();
            log.debug("查询用户最新答案结果，业务层当前登录用户id为：" + userId + ",微博uid:" + uid);
        } catch (Exception e) {
//			return AjaxResult.error("获取用户id失败，请重新登录！");
        ...
        // 查询的问卷类型
        String stype = answerJson.getSurveyType();
        log.info("查询用户最新答案传过来的用户id:{},surveyId:{},stype:{},uid:{}", userId, surveyId, stype, uid);
...
//    	List<UserSurveyResult> list = userSurveyResultService.list(wrapper);
        ...
        if (StringUtils.isNotEmpty(list)) {
            ...
            Map<Long, String> map = list.stream().collect(Collectors.toMap(UserSurveyResult::getUserId,
                    UserSurveyResult::getAnswerResult));
            // 查询用户信息
...
//    	List<UserSurveyResult> list = userSurveyResultService.list(wrapper);
    ...
    public JSONObject getUserDisc(Long userId) {
        ...
        if (StringUtils.isNotEmpty(list)) { // 有测试数据
            ...
            // 查询用户的订单数据
            selectOrder(String.valueOf(userId), String.valueOf(surveyId));
    ...
    public JSONObject aiTest(AiTestVo vo) {
        ...
        if (StringUtils.isEmpty(uid)) {
            log.error("未获取到uId，参数错误！");
            throw new ServiceException("未获取到uId，参数错误！");
        }
        ...
        //明星参数  查找记录  如果上次修改时间距离当天零点少于36小时，不更新，返回存储记录
//        if ("pk_star".equals(vo.getType())) {
//            LambdaQueryWrapper<StarAnswer > wrapper=new LambdaQueryWrapper<>();
//            wrapper.eq(StarAnswer::getStarUid,uid);
//            StarAnswer starAnswer=starAnswerService.getOne(wrapper);
//            long betweenHour = DateUtil.between(DateUtil.beginOfDay(new Date()), starAnswer.getUpdateTime(),
//                    DateUnit.HOUR);
//            if(betweenHour<36){
//                result.put("code", 200);
//                result.put("anId",starAnswer.getStarAnswerId());
//                result.put("param",vo);
//                return result;
//            }
//        }
        ...
        if (null != retest && 1 == retest) { // 第一次请求
            // aid组合
            aid = uid + "-" + surveyId + "-" + System.currentTimeMillis();
            vo.setAid(aid); // 生成队列唯一标识
            // 发送到队列
            activeMQUtils.sendMessageToQueue(QueueDictionary.AI_TEST, vo);
//            modelApi.getMbtiByWb(vo.getStarData(), vo.getUser(), vo.getUserToken(), vo.getStartTime(), vo
//            .getEndTime(), "false",
//                    vo.getNum());
            result.put("code", 201);
        } else { // 后续轮询请求
            ...
            log.info("轮询查询缓存中的数据为：{}:{}", uid, redisJson);
            if (null != redisJson) {
                Integer code = redisJson.getInteger("code");
                String msg = redisJson.getString("msg");
                if (null != code) {
                    if (code == 200) { // 正确结果
//                        if (StringUtils.isNotEmpty(vo.getStarData())) {
//                            JSONObject jsonObject = modelApi.getMbtiByWb(vo.getStarData(), vo.getUser(),
//                                    vo.getUserToken(), null, null, "false", null);
//                            String data = jsonObject.getString("data");
//                            Long anId = redisJson.getLong("data");
//                            if ("updating".equals(data)) {
//                                result.put("msg", "系统正在生成明星AI测试结果，请稍等！");
//                                result.put("code", 203);
//                            } else {
...
//			result.setSurveyId(surveyId);
    ...
    public SurveyOrder selectOrder(String userId, String surveyId) {
        ...
        sowrapper.orderByDesc(SurveyOrder::getCreateTime);
        List<SurveyOrder> orderList = surveyOrderService.list(sowrapper);
        for (SurveyOrder surveyOrder : orderList) {
            JeepayOrder jeepayOrder = jeepayOrderService.selectByMchOrderNo(surveyOrder.getMchOrderNo());
            if (jeepayOrder!=null&&2 == jeepayOrder.getState()) {
                surveyOrder.setStatus(OrderPayStatus.HAVE_PAY);
                surveyOrderService.updateById(surveyOrder);
                return surveyOrder;
            }
        }
        return null;
